...
class TestOrderCreation:
    ...
    def test_order_creation_with_auth_with_ingredients_success(self):
        ...
        response = OrderCreation.order_creation(self, ingredients, user_creation_email_pass[3])
    ...
    def test_order_creation_without_auth_with_ingredients_fail(self):
        ingredients = OrderCreation.order_some_ingredients_get(self)
        response = OrderCreation.order_creation(self, ingredients, "")
        assert (response.status_code == 200 and response.json()["success"] == True and
    ...
    def test_order_creation_with_auth_no_ingredients_fail(self):
        user_creation_data = UserCreation.register_new_user_and_return_email_password(
            self, 10, 10, 10)
        user_creation_email_pass = user_creation_data[0]
        response = OrderCreation.order_creation(self, "", user_creation_email_pass[3])
    ...
    def test_order_creation_with_auth_incorrect_ingredients_fail(self):
        ...
        user_creation_email_pass = user_creation_data[0]
        ingredients = OrderCreation.order_some_ingredients_get(self)
        response = OrderCreation.order_creation(self, ingredients[0] + 'tr', user_creation_email_pass[3])
